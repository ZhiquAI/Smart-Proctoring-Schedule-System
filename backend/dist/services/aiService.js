...
class AIService {
    ...
    async handleRulesUpdate(messages, context) {
...
请根据用户输入，识别意图并提供建议。`;
    ...
    async handleConflictAnalysis(messages, context) {
...
冲突信息：${JSON.stringify(context.conflictInfo, null, 2)}`;
    ...
    async handleScheduleGeneration(messages, context) {
...
用户意图：${context.userIntent}`;
    ...
    async handleRulesExplanation(messages, context) {
...
当前规则：${JSON.stringify(context.currentRules, null, 2)}`;
    ...
    async handleGeneralChat(messages, context) {
    ...
    applyRuleUpdate(currentRules, args) {
